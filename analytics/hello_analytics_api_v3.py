...
def process_results(results):
  ...
  # used to removed special characters from the article name
  parser = HTMLParser.HTMLParser() 
  userDataMap = { }  
#  userDataMap = {'vishal':{}, 'anil':{},'atul' : {} }
  if results.get('rows', []):
    for row in results.get('rows'):
...
def updateUserDatabase(userProfileMap) :
	...
	for section in userProfileMap[user] :
		...
		users.update({'_id': user}, {'$set':{'categories.' + section : [clicks, time + userProfileMap[user][section]]}})
 		totalTime = totalTime + userProfileMap[user][section]
	users.update({'_id': user}, {'$set':{'totalTime' : u['totalTime'] + totalTime}})
